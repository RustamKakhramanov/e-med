    ...
    <div class="col-md-12">
        ...
        <div class="clearfix">
            ...
            <?php if ($model->creation) { ?>
                <span data-url="/<?= Yii::$app->controller->id; ?>/cancel-ajax/<?= $model->id; ?>?back=<?= urlencode(Yii::$app->request->referrer); ?>" class="btn btn-default pull-left ml20 cancel-add-handler">Отменить создание</span>
            <?php } else { ?>
                <span data-url="/<?= Yii::$app->controller->id; ?>/cancel-ajax/<?= $model->id; ?>" class="btn btn-default pull-left ml20 cancel-add-handler">Отменить событие</span>
            <?php } ?>
        </div>
        <hr/>
        ...
        <div class="patients-pick-ctr" style="display: <?= $model->patient_id ? 'none' : 'block'; ?>;">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#<?= $formUid; ?>').on('beforeValidate', function (event, messages, deferreds) {
            $('.patient-create-ctr .form-group').removeClass('has-error');
            $('.patient-create-ctr input').each(function () {
                if ($(this).attr('id')) {
                    $('#<?= $formUid; ?>').yiiActiveForm('remove', $(this).attr('id'));
                }
            });
            return true;
        });
        ...
        $('.tfinder-input .input-handler').autocomplete('instance')._renderMenu = function (ul, items) {
            var that = this;
            console.log(that);
            $.each(items, function (index, item) {
                that._renderItemData(ul, item);
            });
            $(ul).append('<li class="tfinder-empty">' + tfinderTerm + '</li>');
        };



        $(document).off('click', '.tfinder-input .tfinder-empty').on('click', '.tfinder-input .tfinder-empty', function () {
            $(this).hide();
        ...
        $('#<?= $formUid; ?>').on('submit', function () {
            ...
            $.ajax({
                ...
                type: 'post',
                data: {
                    Event: $form.serializeObject().Event,
                    _sended: true
                },
                dataType: 'json',
                success: function (resp) {
        ...
        $('.cancel-add-handler').on('click', function () {
